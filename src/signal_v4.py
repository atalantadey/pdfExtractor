...
def process_page(page_number, image, prev_signals):
    ...
    if prev_signals:
        ...
        if prev_signals.get("page_info_strict") and signals["page_info_strict"]:
            ...
            elif curr_pn == 1 and curr_total is not None and curr_total <= 20 and curr_total > 1:
        ...
        if not continuity_reasons.get("strong_page_number_continuity_strict") and prev_signals.get("page_info_general") and signals["page_info_general"]:
            ...
            elif prev_pn_gen is not None and curr_pn_gen is not None and prev_pn_gen + 1 == curr_pn_gen and prev_total_gen is not None and curr_total_gen is not None and prev_total_gen == curr_total_gen and curr_pn_gen <= 20 and prev_pn_gen <= 20 and curr_total_gen <= 20 and prev_total_gen <= 20:
                weight = max(weight, 0.7)
                continuity_reasons["general_page_number_continuity_with_total"] = True
        ...
        if prev_signals.get("heading") and signals["heading"]:
            ...
            if len(lcs) > 0.8 * min(len(prev_signals["heading"]), len(signals["heading"])):
                ...
                if prev_signals["heading"] and signals["heading"]: # Ensure no None values for similarity calculation
...
def perform_ocr(images):
    ...
    if general_page_numbers:
        ...
        for page, numbers in general_page_numbers.items():
            print(f"  Page {page}: Current - {numbers[0]}, Total - {numbers[1] if numbers[1] is not None else 'N/A'}")
    if date_counts:
