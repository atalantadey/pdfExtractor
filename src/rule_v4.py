...
# Configure logging
logging.basicConfig(level=logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname) - %(message)s'))
...
def group_pages(ocr_data):
    """Groups pages based on strict page numbering and continuity weights."""
    grouped_sections = []
    current_group = []
    ...
    while i < n_pages:
        ...
        if page_info_strict and page_info_strict[0] == 1 and page_info_strict[1] > 1:
            if current_group and continuity_reasons:  # Only add if there were continuity reasons
        ...
        # Priority 2: Use continuity weight for grouping
        if continuity_weight > THRESHOLD:
            current_group.append((page_number, heading_preview, detected_dates))
        else:
            # Start a new group if the continuity weight is low and there's a current group with reasons
            if current_group and continuity_reasons:
                grouped_sections.append(current_group)
                current_group = [(page_number, heading_preview, detected_dates)]
            elif not current_group:
                # If no current group, start one
                current_group = [(page_number, heading_preview, detected_dates)]
            elif not continuity_reasons:
                # If continuity reasons are empty, start a new group
                grouped_sections.append(current_group)
                current_group = [(page_number, heading_preview, detected_dates)]

        i += 1

    # Add any remaining pages if there were continuity reasons
    if current_group and current_group[0][0] != 1 and page_info_strict is None: # Avoid grouping everything at the end if no "Page 1 of N"
        grouped_sections.append(current_group)

    final_sections = []
    for group in grouped_sections:
        if len(group) > 0:  # Keep all non-empty groups
